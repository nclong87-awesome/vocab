...
import { isPhrasalVerb, findWordInCollection, hasUserIncorporatedWord, sanitizeEvaluationWhatWentWell } from "./src/utils/wordNormalization";
...
app.post("/api/generate-challenge", async (req, res) => {
  ...
  try {
    ...
    const rawInterests = personalityProfile?.detectedInterests && personalityProfile.detectedInterests.length > 0
      ? personalityProfile.detectedInterests
      : ["Travel & Dining", "Physics", "programming", "Daily Life & Hobbies"];
    const interests = rawInterests.join(", ");
    const traits = (personalityProfile?.archetypeTraits && personalityProfile.archetypeTraits.length > 0
      ? personalityProfile.archetypeTraits
...
OUTPUT FORMAT (STRICT RAW JSON ONLY):
{
  ...
  "personalityNote": "Short profile-alignment explanation"
    ...
    if (parsed && parsed.nativeSentence && (parsed.idealTranslation || isVietnameseNative)) {
      ...
      const tw = parsed.targetWordFromCollection;
      if (tw?.word && tw?.translation) {
        pairsToReplace.push({ word: String(tw.word).trim(), translation: String(tw.translation).trim() });
      }
      if (Array.isArray(parsed.keyTargetWords)) {
        for (const kw of parsed.keyTargetWords) {
          if (kw?.word && kw?.translation) {
            pairsToReplace.push({ word: String(kw.word).trim(), translation: String(kw.translation).trim() });
          }
        }
      }
      for (const { word, translation } of pairsToReplace) {
        ...
        if (regex.test(parsed.nativeSentence)) {
          parsed.nativeSentence = parsed.nativeSentence.replace(regex, (m: string) => {
...
app.post("/api/challenge-turn", async (req, res) => {
  ...
  try {
    ...
    // Check for incomplete / accidental submission
    const trimmedMsg = rawUserMsg.trim();
    const userWords = trimmedMsg.split(/\s+/).filter(Boolean);
    const idealWords = (challenge.idealTranslation || challenge.nativeSentence || "").split(/\s+/).filter(Boolean);
    const trailingConnectives = ["the", "a", "an", "is", "are", "was", "were", "to", "in", "at", "of", "for", "with", "that", "this", "and", "or", "because", "if", "when", "while", "as", "how", "what", "will", "would", "should", "could", "can", "may", "might"];
  ...
  "evaluation": {
    ...
    if (parsed && (parsed.intent === "assistance" || parsed.intent === "submission")) {
      if (parsed.evaluation) {
        ...
        if (targetWord) {
          // Programmatic gate: The target word or its grammatical forms MUST actually be present in the user text or translation
