        ...
        <div className="p-3.5 sm:p-5 space-y-4 sm:space-y-5 overflow-y-auto">
          ...
          <div className="p-3 sm:p-4 bg-amber-50/90 border-2 border-amber-300 text-stone-900 space-y-2.5 sm:space-y-3">
            ...
            <div className="space-y-2 text-xs">
              ...
              {/* Removed Words (Deletion Synced) */}
              {diff && diff.deletedWordsToSync && diff.deletedWordsToSync.length > 0 && (
                <div className="space-y-1 bg-white p-2 sm:p-2.5 border border-amber-200">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-rose-800 flex items-center gap-1 text-[11px] sm:text-xs">
                      <Trash2 className="w-3.5 h-3.5 text-rose-600 shrink-0" />
                      Removed Words Synced (-{diff.deletedWordsToSync.length})
                    </span>
                    <span className="text-[9px] sm:text-[10px] text-stone-500 font-mono">Cloud & Local</span>
                  </div>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {diff.deletedWordsToSync.map(w => (
                      <span key={w.id} className="text-[10px] sm:text-[11px] font-semibold bg-rose-50 text-rose-900 border border-rose-200 px-1.5 sm:px-2 py-0.5 font-serif line-through decoration-rose-500">
                        {w.word}
                      </span>
                    ))}
                  </div>
