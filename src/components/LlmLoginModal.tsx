...
export default function LlmLoginModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-2.5 sm:p-4 bg-stone-950/75 backdrop-blur-sm overflow-y-auto">
      <motion.div
        initial={{ opacity: 0, scale: 0.96, y: 10 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.96, y: 10 }}
        transition={{ duration: 0.2 }}
        className="bg-white border border-stone-200 shadow-2xl max-w-2xl w-full my-auto max-h-[90vh] sm:max-h-[86vh] flex flex-col overflow-hidden"
        id="llm-login-modal"
      >
        {/* Header Bar */}
        <div className="bg-stone-900 text-white p-3.5 sm:p-5 flex justify-between items-start border-b border-stone-800 shrink-0">
          <div>
            <div className="inline-flex items-center gap-1.5 text-[10px] font-bold tracking-widest uppercase bg-stone-800 text-stone-300 px-2 py-0.5 mb-1.5">
            ...
            <p className="text-stone-400 text-[11px] sm:text-xs mt-0.5 font-serif italic leading-relaxed">
          ...
          {canDismiss && onClose && (
            <button 
              onClick={onClose}
              className="p-1.5 text-stone-400 hover:text-white transition-colors ml-2 shrink-0"
        ...
        {/* Content Form - Internal Scroll Area */}
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-3.5 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center gap-2">
              ...
              <button
                ...
                className="text-[11px] sm:text-xs font-medium text-stone-600 hover:text-stone-950 underline underline-offset-2 shrink-0"
              >
            ...
            {isCustomModelMode ? (
              <input
                ...
                className="w-full bg-stone-50 border border-stone-300 p-2.5 sm:p-3 text-xs text-stone-900 font-mono focus:outline-none focus:border-stone-900"
              />
            ) : (
              <select
                value={model}
                onChange={(e) => setModel(e.target.value)}
                className="w-full bg-stone-50 border border-stone-300 p-2.5 sm:p-3 text-xs text-stone-900 font-medium focus:outline-none focus:border-stone-900 cursor-pointer"
              >
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-stone-50 border border-stone-300 p-2.5 sm:p-3 pr-10 text-xs text-stone-900 font-mono focus:outline-none focus:border-stone-900"
              />
              <button
                type="button"
          ...
          {testingStatus !== "idle" && (
            <div className={`p-3 sm:p-3.5 text-xs flex items-center gap-3 border ${
          ...
          {/* Action Buttons */}
          <div className="flex flex-col-reverse sm:flex-row justify-between items-stretch sm:items-center gap-2.5 pt-4 border-t border-stone-200 shrink-0">
            <button
              type="button"
              onClick={handleTestConnection}
              disabled={testingStatus === "testing"}
              className="px-4 sm:px-5 py-2.5 sm:py-3 border border-stone-300 hover:border-stone-900 bg-stone-50 hover:bg-stone-100 text-stone-900 font-semibold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer"
            >
              {testingStatus === "testing" ? (
                <>
            ...
            <div className="flex items-center gap-2">
              {canDismiss && onClose && (
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 sm:flex-none px-4 sm:px-5 py-2.5 sm:py-3 border border-stone-200 text-stone-600 hover:text-stone-900 font-medium text-xs transition-colors cursor-pointer text-center"
                >
                  Cancel
                </button>
              )}
              <button
                type="submit"
                className="flex-1 sm:flex-none px-6 sm:px-8 py-2.5 sm:py-3 bg-stone-900 hover:bg-black text-white font-semibold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer shadow-xs hover:shadow-sm"
              >
