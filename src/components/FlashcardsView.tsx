...
export default function FlashcardsView({
  ...
  const handleFlashcardStrengthUpdate = (word: Word, delta: number, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    if (!onUpdateWords) return;

    const oldStrength = word.strength ?? 0;
    ...
    const note = actualDelta >= 0 
      ? `Flashcard Recall Practice (+${actualDelta}%)`
      : `Flashcard Recall Practice (${actualDelta}%)`;

    const updatedWord = recordStrengthHistory(word, newStrength, 'flashcard_review', note);
    const updatedWords = words.map((w) => (w.id === word.id ? updatedWord : w));

    onUpdateWords(updatedWords);
    saveAllWordsToDB(updatedWords).catch((err) => console.error("Error saving word strength:", err));
  };
              ...
              {currentWord && (
                ...
                >
                  ...
                  <div className="w-full min-h-[400px] sm:min-h-[440px] bg-white border border-stone-200 flex flex-col justify-between p-4 sm:p-6 relative">
                    ...
                    {/* Memory Strength & Recall Practice Controls */}
                    <div 
                      className="w-full bg-stone-900 text-white p-3 rounded-xl border border-stone-800 space-y-2 mt-2 z-10"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div className="flex items-center justify-between gap-2 flex-wrap text-xs">
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            type="button"
                            onClick={(e) => handleFlashcardStrengthUpdate(currentWord, 10, e)}
                      ...
                      <div className="w-full bg-stone-800 h-1.5 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.max(0, Math.min(100, currentWord.strength || 0))}%` }}
                        />
                      </div>
                    </div>
