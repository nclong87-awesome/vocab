...
## 8. Client-Side Request & Response Persistence in Local IndexedDB

To ensure complete operational visibility, enable rapid debugging, and provide users with a local audit trail of all AI interactions without server database costs, the client application persists every outbound request and inbound response from Cloudflare Worker microservices directly into the browser's local **IndexedDB** (`apiLogs` store).

### 8.1 Data Model: `ApiRequestLog`

The log entry structure captures prompt details, execution parameters, latency, status codes, and the raw payload:
...
### 8.2 IndexedDB Database Layer (`src/db/indexedDB.ts`)

The database persistence engine uses transactional IndexedDB operations with automated FIFO trimming to keep storage bounded:
...
### 8.3 Action Classification & Service Wrapper (`src/services/requestHistoryService.ts`)

Before saving to IndexedDB, prompts and responses are analyzed to automatically categorize the functional feature:
...
export function detectActionCategory(
  ...
  if (combined.includes('status": "connected"') || combined.includes("/api/test-llm")) {
    return "Connection Test";
  }
  if (combined.includes("evaluate a language learner's translation attempt") || combined.includes("challenge-turn")) {
    return "Challenge Evaluation";
  }
  if (combined.includes("translation challenge tutor") || combined.includes("ask-ai-challenge")) {
    return "Challenge Ask AI";
  }
  if (combined.includes("translation challenge") || combined.includes("generate-challenge")) {
...
### 8.4 Integration with Cloudflare Worker Client Dispatch (`src/services/llmClientService.ts`)

Whenever the client initiates an HTTP request to any Cloudflare Worker proxy (`https://groq.nclong87.workers.dev`, `https://openrouter.nclong87.workers.dev`, etc.), the request and response are automatically intercepted and persisted into IndexedDB:
...
export async function callCloudflareWorkerWithLogging(params: {
  ...
  try {
    ...
    const response = await fetch(`${params.workerUrl.replace(/\/+$/, '')}/chat/completions`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Proxy-Key": params.accessKey,
...
### 8.5 Key Benefits of IndexedDB Request & Response Persistence
1. **Zero Server Overhead**: The entire audit history resides safely in the user's browser, eliminating external database egress and storage fees.
2. **Instant Debugging & Transparency**: Users and developers can open the **Request History Modal** to view exact prompts, raw JSON responses, token outputs, and HTTP error traces.
3. **Adaptive Auto-Pruning**: Uses FIFO eviction to maintain between 100 and $\approx 1000$ entries, preventing browser storage exhaustion.
4. **Resilience & Offline Audit**: Historical logs remain fully accessible even when offline or during transient network outages.
